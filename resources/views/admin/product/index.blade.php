          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="width: 25%;">
                ...
                @endif
                @if(($product->status) == 1)
                <a onclick="return confirm('Bạn có muốn thay đổi trạng thái sản phẩm này')"
                  href="admin/products/off/{{$product->id}}" class="btn btn-sm btn-danger">Tắt</a>
                @else
                <a onclick="return confirm('Bạn có muốn thay đổi trạng thái sản phẩm này')"
                  href="admin/products/on/{{$product->id}}" class="btn btn-sm btn-success">Bật</a>
                @endif
